...
public class LinkedBST<E extends Comparable<E>> implements BinarySearchTree<E> {
    ...
    private int contarHojas(Nodo<E> nodo) {
        if (nodo == null) return 0; // si no hay nada, no hay hojas
        if (nodo.izq == null && nodo.der == null) return 1; // si no tiene hijos es hoja
        return contarHojas(nodo.izq) + contarHojas(nodo.der); // sumamos hojas izq y der
    }
}